...
public abstract class SQLiteJobDataRepository implements JobsDataRepository {

    private final String tableName = cf.getString("FILENAME", "jobs"); // Nome fixo da tabela para SQLite
    private final Connection connection;

    public SQLiteJobDataRepository(Connection connection) {
    ...
    public void setPlayerProfession(UUID uuid, String name, String profession) {
        try {
            if (hasPlayer(uuid)) {
                String query = String.format("UPDATE %s SET name = ?, profession = ? WHERE uuid = ?", tableName);
                ...
                String query = String.format("INSERT INTO %s (uuid, name, profession, working) VALUES (?, ?, ?, ?)", tableName);
                PreparedStatement ps = connection.prepareStatement(query);
                ps.setString(1, uuid.toString());
                ps.setString(2, name);
                ps.setString(3, profession);
                ps.setString(4, "false"); // Valor padrão para working
                ps.executeUpdate();
            }
